...
SearchDataDictionary <- function(DataDictionaryList, QueryString){
  ...
  # Output: A list of data frames (data dictionaries) displaying any rows that have a match with QueryString
  ...
  # save only the working data dictionaries; unable to read in meta data for 18m and 24m and for echo 
  # InfantDataDicts <- InfantDataDicts[1:5]
  
  # consolidate all data dictionaries
  ...
  #                   "obstetric_history", "follow_up_measures", "s_t_a_i", "c_e_s_d_scale")
  ...
  # Handle word boundaries and word characters cases for exact word matching
  
  #####################################################################
  
  # Split queried string into individual words 
  CollapsedQueryString <- unlist(str_split(QueryString, pattern = " "))
  
  if(length(CollapsedQueryString) > 1){
    
    # Handle word boundaries case:
    
    # add word boundaries in between each string in vector
    QueryStringBoundary <- paste(CollapsedQueryString, collapse = "\\b|\\b")
    
    # 3 cases: words with boundaries on both side, words with boundaries on only left side and only right side
    # add word boundaries to beginning and end of string
    QueryStringBothBoundary <- paste0("\\b", QueryStringBoundary, "\\b")
    QueryStringRightBoundary <- paste0(CollapsedQueryString, "\\b")
    QueryStringLeftBoundary <- paste0("\\b", CollapsedQueryString)
    
    
    
    # Handle word character case (i.e: words in between underscores or hypens):
    
    # add word character in between each string in vector
    QueryStringWordChar <- paste(CollapsedQueryString, collapse = "\\w|\\w")
    
    
    # 3 cases: words with word characters on both side, words with word characters on only left side and only right side
    # add word character to beginning and end of single collapsed string
    QueryStringBothWordChar <- paste0("\\w", QueryStringWordChar, "\\w")
    # need to handle case of "hello_bye_morning". Code is matching "bye", but not "hello" or "morning"
    QueryStringRightWordChar <- paste0(CollapsedQueryString, "\\w")
    QueryStringLeftWordChar <- paste0("\\w", CollapsedQueryString)
    ...
    QueryStringBothBoundary <- paste0("\\b", CollapsedQueryString, "\\b")
    QueryStringRightBoundary <- paste0(CollapsedQueryString, "\\b")
    QueryStringLeftBoundary <- paste0("\\b", CollapsedQueryString)
    
    # Word character case
    
    QueryStringBothWordChar <- paste0("\\w", CollapsedQueryString, "\\w")
    # need to handle case of "hello_bye_morning". Code is matching "bye", but not "hello" or "morning"
    QueryStringRightWordChar <- paste0(CollapsedQueryString, "\\w")
    QueryStringLeftWordChar <- paste0("\\w", CollapsedQueryString)
  }
  
  # combine both cases into one string regex
  QueryString <- paste(c(QueryStringBothBoundary, QueryStringRightBoundary, QueryStringLeftBoundary,
                         QueryStringBothWordChar, QueryStringRightWordChar, QueryStringLeftWordChar), 
                       collapse = "|")
  ...
  FilteredDataDictionary <- map(DataDictionaryList,
                                ~filter_all(.x, any_vars(
                                  str_detect(., regex(QueryString, ignore_case = TRUE))))) %>% 
